...
export default function GroupChat({ avatar, name, info, messages: initialMessages }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-white max-w-md mx-auto">
      ...
      {/* Header */}
      <motion.div className="flex items-center p-4 border-b border-gray-200" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <div className="ml-auto">
          <Link href="/mobile/group-profile">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 relative" ref={messagesContainerRef} style={{ scrollBehavior: "smooth" }}>
        <AnimatePresence>
      ...
      {/* Input */}
      <motion.div className="p-3 border-t border-gray-200" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }}>
        <div className={`flex items-center bg-gray-100 rounded-full px-4 py-2 ${isInputFocused ? "ring-2 ring-blue-300" : ""}`}>
          ...
          />
          {inputValue ? (
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="text-blue-500 ml-2" onClick={handleSendMessage}>
              <Send size={20} />
            </motion.button>
          ) : (
