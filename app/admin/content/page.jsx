...
const ContentPage = () => {
  ...
  async function fetchVideos() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/content?page=${page}&limit=${videosPerPage}`);
  ...
  return (
    <div className="content-page p-4 md:p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        ...
        <div className="flex gap-4 mt-4 md:mt-0">
          ...
          <Link href="/admin/content/instagram">
            <button className="flex items-center gap-2 bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500  px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      ...
      {/* Search and Tabs */}
      <div className=" rounded-lg shadow p-4 md:p-6 mb-6">
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredVideos.map((video) => (
                <div
                  key={video.id}
                  className={`video-item  rounded-lg overflow-hidden hover:shadow-lg transition-shadow ${
