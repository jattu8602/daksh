...
export default function SmartBulkUploadPage() {
    ...
    const fetchClassData = async () => {
      try {
        ...
        if (data.success) {
          ...
          const nextRollNo =
            (data.class.totalStudents || 0) + (data.class.startRollNumber || 1)
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(
        `/api/schools/${schoolId}/classes/${classId}/students/smart-bulk/upload`,
        {
          method: 'POST',
          body: formData,
        }
      ...
      if (result.successfulImports > 0) {
        ...
        setTimeout(() => {
          router.push(
            `/admin/schools/${schoolId}/classes/${classId}?refresh=true`
          )
        }, 1500)
      }
    } catch (error) {
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="text-sm breadcrumbs">
        <ul className="flex items-center space-x-2 text-gray-600">
          ...
          <li className="flex items-center">
            <span className="mx-2">/</span>
            <span className="text-gray-900 font-medium">Smart Bulk Import</span>
          </li>
        </ul>
      </div>
      ...
      <div className="bg-white rounded-lg border p-6 shadow-sm">
        <h1 className="text-2xl font-bold">
          Smart Bulk Import for{' '}
          {classData.parentClass ? classData.parentClass.name : classData.name}
          {classData.section && ` (${classData.section})`}
        </h1>
        <p className="text-gray-600 mt-1">
          Efficiently add multiple students using an Excel template.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg border p-6 shadow-sm">
          <h2 className="font-semibold text-lg flex items-center">
            <span className="bg-gray-200 text-gray-800 rounded-full h-8 w-8 text-sm flex items-center justify-center mr-3">
              1
            </span>{' '}
            Download Template
          </h2>
          <p className="text-gray-600 my-4">
            Get a pre-filled Excel template with the correct columns and
        ...
        <div className="bg-white rounded-lg border p-6 shadow-sm">
          ...
          <p className="text-gray-600 my-4">
            Once you've filled out the template, upload it here for processing.
          </p>
          <div className="mt-4">
            <label
              htmlFor="file-upload"
            ...
            <div className="py-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStartRollNumber(parseInt(e.target.value))}
