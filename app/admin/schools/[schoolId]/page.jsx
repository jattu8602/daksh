...
export default function SchoolDetailPage() {
  ...
  // Utility function to get consistent class display name
  const getClassDisplayName = (cls) => {
    if (!cls) return ''
    const baseName = cls.parentClass ? cls.parentClass.name : cls.name
    return cls.section ? `${baseName} (${cls.section})` : baseName
  }
  ...
  // Fetch template classes on component mount
  useEffect(() => {
    const fetchTemplateClasses = async () => {
      try {
        const response = await fetch('/api/classes?isCommon=true')
        const data = await response.json()
        if (data.success) {
          setAllClasses(data.classes)
          console.log('Template classes for dropdown:', data.classes)
        }
      } catch (error) {
        console.error('Error fetching template classes:', error)
      }
    }
  ...
  const handleAddClass = async (e) => {
    ...
    try {
      ...
      const classesResponse = await fetch(
        `/api/schools/${schoolId}/classes?no-cache=true`
      )
  ...
  const handleEditClassClick = (cls) => {
    setClassToEdit(cls)
    setClassFormData({
      classId: cls.parentClassId || cls.id, // Use parent class ID if available, otherwise use current class ID
  ...
  const handleEditClassSubmit = async (e) => {
    ...
    try {
      ...
      // Update the class in the list with proper data transformation
      const updatedClass = {
        ...data.class,
        name: data.class.parentClass
          ? data.class.parentClass.name
          : data.class.name,
        parentClass: data.class.parentClass,
      }

      setClasses(
        classes.map((cls) => (cls.id === classToEdit.id ? updatedClass : cls))
      )
      ...
      if (typeof window !== 'undefined') {
        localStorage.removeItem(`class:${schoolId}:${classToEdit.id}`)
        localStorage.removeItem(`class:${schoolId}:${classToEdit.id}:timestamp`)
      }

      // Show different success message based on whether students were affected
      if (classToEdit.totalStudents > 0) {
        toast.success(
          `Class updated successfully! ${classToEdit.totalStudents} students' usernames and QR codes have been updated to reflect the new class template.`
        )
      } else {
      ...
      // Force refresh of class detail pages by updating URL parameters
      if (typeof window !== 'undefined') {
        // Update the current URL to trigger refresh on class detail pages
        const currentUrl = new URL(window.location.href)
        currentUrl.searchParams.set('classUpdated', 'true')
            ...
            <form onSubmit={handleEditClassSubmit}>
              <div className="mb-4">
                ...
                <p className="mb-2 text-xs text-gray-500">
                  Choose from available template classes
                </p>
                <Select
                  name="classId"
                  value={classFormData.classId}
                  onValueChange={(value) =>
                    setClassFormData({ ...classFormData, classId: value })
                  }
                  disabled={isLoading}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select a template class" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {/* Warning about class change */}
              {classToEdit && classToEdit.totalStudents > 0 && (
                <div className="mb-4 rounded-md bg-yellow-50 p-3 text-sm text-yellow-700">
                  <p className="font-medium">
                    ⚠️ This class has {classToEdit.totalStudents} students.
                  </p>
                  <p className="mt-1">
                    Changing the class template will automatically update all
                    students' usernames, regenerate their QR codes, and update
                    their access to subjects/content for the new class template.
                  </p>
                  <p className="mt-1 text-xs">
                    Example: Student "jattu_2342323_12thpcm_1" will become
                    "jattu_2342323_12thpcb_1" if changing from PCM to PCB.
                  </p>
                  <p className="mt-1 text-xs">
                    Note: Old QR codes will become invalid and new ones will be
            ...
            <div className="mb-6">
              ...
              {classToDelete.totalStudents > 0 && (
                <div className="mt-2 rounded-md bg-red-50 p-3 text-sm text-red-600">
                  <p className="font-medium">
                    Warning: This class has {classToDelete.totalStudents}{' '}
                    students.
                  </p>
                  <p className="mt-1">
          ...
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow">
            ...
            {newClass ? (
              <div className="mb-6 rounded-lg bg-blue-50 p-4 text-sm">
                ...
                <p>
                  <span className="font-medium">Name:</span> {newClass.name}
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow">
            ...
            <div className="mb-6 text-sm text-gray-600">
              <p>
                This will export data for all students in all classes,
                including:
              </p>
              ...
              <p className="mt-2 text-red-500 font-medium">
                Note: This file will contain sensitive information for all
                students. Please ensure you're authorized to access this data.
              </p>
            </div>
