...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if(headerErrors.length > 0) {
        ...
        return NextResponse.json({ error: "Invalid template format", details: headerErrors }, { status: 400 });
    }
    ...
    for (let i = 2; i <= worksheet.rowCount; i++) {
        ...
        const imageUrl = row.getCell(keyMap.image).value?.text || row.getCell(keyMap.image).value;
        ...
        if (!rollNumber || !Number.isInteger(rollNumber)) {
            report.errorList.push({ row: i, error: "Roll Number must be a valid integer." });
            continue;
        }
        ...
        if (!gender || !["Male", "Female", "Other"].includes(gender)) {
            report.errorList.push({ row: i, error: "Gender must be one of 'Male', 'Female', or 'Other'." });
            continue;
        }
        if (existingRollNumbers.includes(rollNumber) || studentsToCreate.some(s => s.rollNo === rollNumber)) {
            report.errorList.push({ row: i, error: `Duplicate Roll Number: ${rollNumber}` });
            continue;
        }

        // Prepare student data
        const cleanName = name.toLowerCase().replace(/\s+/g, '');
        const username = `${cleanName}_${classData.school.code.toLowerCase()}_${classData.name.toLowerCase().replace(/\s+/g, '')}_${rollNumber}`;
        const password = Math.random().toString(36).substring(2, 15);
        const qrData = JSON.stringify({ username, password, classId, rollNo: rollNumber });
    ...
    console.log(`Step 6: Success. Found ${studentsToCreate.length} valid students to create.`);
    ...
    report.errorList.push({ row: 'General', error: error.message || "An unexpected error occurred." });
