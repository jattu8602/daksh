...
export async function POST(request) {
  try {
    ...
    console.log(`Processing channel URL: ${cleanUrl}`)
    ...
    if (cleanUrl.includes('/channel/')) {
      ...
      console.log(`Extracted custom username: ${username}`)
    } else if (cleanUrl.includes('/@')) {
      // Handle format - extract the handle
      username = cleanUrl.split('/@')[1].split('/')[0].split('?')[0]
      // Keep the handle as-is for YouTube handles (preserve underscores, etc.)
      // Only remove trailing dots if present
      username = username.replace(/\.$/, '')
      console.log(`Extracted handle: ${username}`)
      ...
      // Try as username
      username = cleanUrl
      console.log(`Treating as username: ${username}`)
    }

    // If we have a username, get the channel ID
    if (username) {
      console.log(
        `Attempting to fetch channel for username/handle: ${username}`
      )
      try {
        ...
        console.log(`forHandle API response:`, data)
        ...
        if (data.items && data.items.length > 0) {
          ...
          console.log(`forUsername API response:`, usernameData)
              ...
              return NextResponse.json(
                {
                  ...
                  details: `The provided username or handle "${username}" was not found. Tried: forHandle, forUsername, and search methods.`,
        ...
        return NextResponse.json(
          {
            ...
            details:
              'The provided channel ID is invalid or the channel does not exist',
          },
          { status: 404 }
        )
      }
        ...
        try {
          ...
          if (videoIds.length > 0) {
            ...
            if (videoData.items) {
              ...
              console.log(
                `Processed page ${pageCount + 1}, found ${validVideos.length} shorts in this page`
              )
            }
          }
