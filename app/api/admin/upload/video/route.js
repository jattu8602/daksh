...
export async function POST(request) {
  try {
    const { title, description, metaDescription, hashtags, jobId } = await request.json();
    if (!title || !jobId) {
    ...
    if (!job || !job.result || !job.result.metadata || !job.result.metadata.localPath) {
      return NextResponse.json({ error: 'Invalid jobId or missing localPath' }, { status: 400 });
    }
    const { localPath, url } = { ...job.result.metadata, url: job.url };

    // Upload to Cloudflare
    const { cloudflareUrl } = await uploadToCloudflare({ localPath, title, description });
    ...
    await prisma.videoJob.update({ where: { id: jobId }, data: { status: 'uploaded' } });

    return NextResponse.json({ success: true, video });
  } catch (error) {
    console.error('Error uploading video:', error);
    return NextResponse.json({ error: 'Failed to upload video', message: error.message }, { status: 500 });
  }
}